...
function renderProductRows(canEdit) {
  if (state.products.length === 0) {
    return `<tr><td colspan="5" class="text-center text-slate-400 py-8">Belum ada produk.</td></tr>`;
  }
  ...
  return state.products.map(p => `
    <div class="p-4 flex items-center justify-between gap-3">
      <div class="min-w-0">
        ...
        <p class="text-sm font-bold text-brand-700 font-mono-num mt-0.5">${formatRupiah(p.price)}</p>
      </div>
      ${canEdit ? `
      ...
      <div class="space-y-3">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            ...
            <input name="price" type="number" min="0" required value="${product?.price ?? ''}"
              class="w-full h-11 mt-1 px-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-500 touch-target" />
          </div>
          <div>
